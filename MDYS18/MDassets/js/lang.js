...
var zh_style_active = 'color: rgb(254 98 142 / var(--tw-text-opacity));'; //当前选择的链接式样
...
//自动操作后的提示消息
var zh_autoLang_msg =
	'歡迎來到本站,本站爲方便台灣香港的用戶n1.采用UTF-8國際編碼,用任何語言發帖都不用轉碼.n2.自動判斷繁體用戶,顯示繁體網頁n3.在網頁最上方有語言選擇,如果浏覽有問題時可以切換n4.本消息在cookie有效期內只顯示一次';
...
function zh_tranBody(obj) {
	...
	for (var i = 0; i < o.length; i++) {
		...
		if (c.tagName == "INPUT" && c.value != '' && c.type != 'text' && c.type != 'hidden' && c.type != 'password') c
			.value = c.value.tran();
		if (c.nodeType == 3) {
			c.data = c.data.tran();
		} else {
